      ...
      <div className="flex flex-wrap items-center gap-4">
        <nav className="order-3 flex w-full flex-wrap items-center gap-1 border-t border-[#2C303D] pt-3 sm:order-2 sm:flex-nowrap sm:w-auto sm:flex-1 sm:justify-between sm:border-0 sm:pt-0" aria-label="Secciones">
          <div className="flex shrink-0 items-center gap-1">
            {tabButton('main', 'Principal')}
            {tabButton('library', 'Biblioteca')}
            {tabButton('settings', 'Configuración')}
            {tabButton('logs', 'Logs')}
          </div>

          {/* Indicadores a la derecha con menú on hover */}
          <div className="group relative flex shrink-0 items-center gap-3 border-l border-[#2C303D] pl-3 max-sm:w-full max-sm:border-l-0 max-sm:pl-0 max-sm:pt-2 max-sm:border-t max-sm:mt-1">
            <div className="flex items-center gap-1.5 text-xs text-[#8D93A6]">
              <CheckCircle2 size={13} className="text-[#7FD8CC]" />
              <span style={{ fontFamily: FONT_MONO }}>{libraryCount}</span>
            </div>
            <div className="flex items-center gap-1.5 text-xs">
              <LoaderCircle size={13} className={pendingDownloadCount ? 'animate-spin text-[#FFFFFF]' : 'text-[#565C6E]'} />
              <span className={pendingDownloadCount ? 'text-[#FFFFFF]' : 'text-[#565C6E]'} style={{ fontFamily: FONT_MONO }}>
